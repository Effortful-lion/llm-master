---
title: Claude宣布给文字打上隐形水印：复制粘贴也会跟着走，Claude Code、API和C2PA内容凭证全面解析
description: Anthropic为履行欧盟AI法案生成内容透明度承诺，宣布给Claude生成文本嵌入机器可读的隐形水印，并为SVG、PNG、JPG等文件附加C2PA签名来源元数据。本文拆解Claude文字水印的适用模型、Claude Code与API覆盖范围、复制粘贴和编辑后的表现、检测机制及误判边界。
keywords: [Claude隐形水印, Claude文字水印, AI生成内容水印, Anthropic, Claude Code, Claude API, Claude Cowork, Claude Tag, 欧盟AI法案, EU AI Act, Article 50, AI生成内容透明度, 机器可读标记, C2PA, Content Credentials, 内容凭证, AI检测, 大模型水印]
tags: [Claude, Anthropic, AI生成内容, 欧盟AI法案, C2PA, 大模型动态]
---
...
- 使用的是尚未支持标记的旧模型。
- 文字被大幅改写、转述、翻译或混入其他内容。
- 段落太短，信号不足以可靠检测。
- 文件经过格式转换、重新保存或截图，来源元数据被剥离。
- 使用的平台、功能或文件类型不支持对应标记。
