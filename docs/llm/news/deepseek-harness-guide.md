---
title: DeepSeek Harness安装与使用教程：官方dsh怎么配置API、操作代码仓库、跑Headless任务和高效使用
description: DeepSeek官方开源DeepSeek Harness（dsh）。本文讲清DeepSeek Harness是什么、能解决哪些Agent落地问题，并给出Node.js环境准备、Web UI安装、DeepSeek API配置、工作区选择、提示词写法、Headless自动化、Python SDK和高效使用方法。
keywords: [DeepSeek Harness, DeepSeek Harness安装, DeepSeek Harness教程, DeepSeek dsh, dsh安装, dsh使用, DeepSeek Agent, DeepSeek编程, DeepSeek代码助手, AI编程, Coding Agent, Agent Harness, Headless Agent, DeepSeek API, DeepSeek V4, Cordis, AI Agent框架, Python SDK, Agent插件]
tags: [DeepSeek, DeepSeek Harness, AI Agent, AI编程, Agent工程]
---

# DeepSeek官方Harness怎么用：从安装到让它真正改完一个项目

前面写 [DeepSeek招聘Agent Harness工程师](./deepseek-agent-harness-hiring.md) 时，我们讲的是一个行业变化：**模型负责推理，Harness负责把推理接到文件、命令、状态、权限和验证上。**

当时不少录友问：概念我懂了，但 Harness 到底长什么样？普通人能不能直接用？

现在 DeepSeek 自己把答案开源了。

[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 的命令叫 `dsh`。它不是再套一层聊天页面，而是一套能读写工作区、运行命令、维护计划、调用工具并持续执行任务的 Agent Harness。
...
进入 [DeepSeek开放平台](https://platform.deepseek.com/api_keys) 创建 API Key，并确认账户有可用余额。
...
**Agent说完成，不算完成；验证器通过，才算完成。** 这也是 [Agent为什么容易翻车](../app/agent_failure_modes.md) 里一直强调的问题。
...
不是站队，是算账：**先按任务难度分流，再看失败一次要返工多少。** 关于模型和 Harness 的关系，可以接着看 [DeepSeek V4](./deepseek-v4.md)。
...
完整代码以官方的 [Python SDK指南](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/user/guide/python-sdk.md) 为准。Developer Preview 阶段接口变化快，文章里复制一份长代码，很快就会过期。
...
真正要开发插件，再阅读官方的 [架构文档](https://github.com/deepseek-ai/deepseek-harness/blob/master/docs/architecture.md) 和 [插件开发教程](https://github.com/deepseek-ai/deepseek-harness/tree/master/docs/user/develop/basic)。
