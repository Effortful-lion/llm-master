---
title: GLM-5.2发布：智谱这次不放跑分表，先让你用上，开源、1M上下文、华为昇腾训练，和Opus 4.8、DeepSeek V4怎么选
description: 智谱于2026年6月13日发布GLM-5.2，最反常的地方是发布当天没有放任何benchmark跑分表，主打Coding Plan优先开放、MIT开源、可用的1M上下文和华为昇腾训练。本文从"不放跑分"的发布策略、开源与国产算力、GLM Coding Plan四个档位怎么买、以及GLM-5.2和Claude Opus 4.8、DeepSeek V4、MiniMax M3怎么选几个角度，拆解这次发布到底值不值得跟。
keywords: [GLM-5.2, GLM5.2, GLM-5.2发布, 智谱GLM-5.2, GLM Coding Plan, GLM-5.1, GLM-5, 开源大模型, 国产大模型, 华为昇腾, 1M上下文, SWE-bench, Terminal-Bench, Claude Opus 4.8, DeepSeek V4, MiniMax M3, AI编程, Agent, Claude Code替代, 大模型发布]
tags: [GLM, 智谱, 大模型发布, AI编程, Agent, 开源大模型]
---
...
我们前面写 [Claude Opus 4.8](./claude-opus-4-8.md) 那篇就吐槽过：**大表负责传播，脚注负责真相。** 一个模型换个 harness、换个工具链，分数能差好几个点。
...
我们在 [MiniMax M3 评测](./minimax-m3.md) 里反复强调过这点：长上下文的"能装"和"装了还准"差很远。
...
GLM-5.2 现在的入口就是 [GLM Coding Plan](https://www.bigmodel.cn/glm-coding?ic=L9I20DKWAG)。
...
我在 [DeepSeek V4 降价](./deepseek-v4-price.md) 那篇就提过，目前我写代码性价比最高的组合之一，就是 Claude 的 Agent + CLI，配 GLM 的 Coding Plan 兜底。
