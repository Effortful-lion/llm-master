---
title: GPT-5.6发布后，Superpowers已经没必要学了？Agent Skill、Claude Code、Codex开发流程该做减法了
description: GPT-5.6发布后，模型已经更擅长理解意图、自主规划、调用工具和持续完成任务。本文分析为什么Superpowers和大量通用Agent Skill不再适合默认启用，哪些Skill可以删除，哪些项目规则、确定性脚本和高风险流程仍然值得保留，并给出先裸跑、看失败样本、再补最小Skill的评测方法。
keywords:
  - GPT-5.6
  - GPT-5.6 Sol
  - Superpowers
...
[GPT-5.6发布](./gpt-5-6.md)之后，模型自己理解需求、规划步骤、调用工具、运行测试和检查结果的能力又往前走了一大截。
...
[Superpowers](https://github.com/obra/superpowers)不是一个让模型突然变聪明的魔法插件。

它是一套软件开发方法论，把开发过程拆成一组可以自动触发的Skill：

- 动手前先澄清需求、做brainstorming
- 设计确认后再写实施计划
- 按TDD推进，而不是先写完再补测试
- 把任务交给子Agent执行和审查
- 完成前必须验证，不能嘴上说“应该没问题”
...
我前面在[AI增强开发三件套](../../interview/llm/ai_enhanced_development_openspec_superpowers_gstack.md)里还专门介绍过它。
...
OpenAI在[GPT-5.6官方使用指南](https://developers.openai.com/api/docs/guides/latest-model)里明确提到，模型现在更擅长从上下文理解用户真实意图，很多时候已经不需要人把每一步都规定出来。
...
OpenAI目前仍然把Skill定义为可复用工作流，并通过渐进式加载避免一次性塞满上下文，详见[官方Skill文档](https://learn.chatgpt.com/docs/build-skills)。
...
这和[Agent Skill面试详解](../../interview/llm/agent_skill_interview.md)里讲的原则并不矛盾。
...
如果你还在用GPT-5.5时期的长Prompt，可以先看[GPT-5.6 Sol Prompt指南](../intro/gpt56_sol_prompt_guide.md)。
